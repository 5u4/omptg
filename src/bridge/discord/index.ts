...
const DISCORD_SYSTEM_BLOCK = [
	...
	"  - GFM tables (`|`/`---` rows show as literal text) — NEVER use tables;",
	"    render tabular data as `- **key**: value` bullets, short lists, or",
	"    a fenced code block when columns truly matter.",
	...
	"The bridge rewrites the rest to the closest supported form (H4+ →",
	"bold, images → plain link, task list → `☐`/`☑`, HR → `———`), but",
	"prefer the native form when you can: a short bold lead-in reads",
	"better than a deep heading.",
